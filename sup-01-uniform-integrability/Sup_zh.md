...
这就是**一致可积性**（uniform integrability）。
...
固定一个很大的 $K$ 后，每个函数都可能存在超过 $K$ 的部分。但如果函数族一致可积，那么只要 $K$ 足够大，这些部分所携带的积分质量就能够**同时**变得任意小。
...
因此它并不是“每个函数都可积”的另一种写法，而是一个真正的**函数族性质**。概率论中也通常以完全相同的尾积分形式定义 uniform integrability。
...
它更接近于一种**紧性条件**：它控制一个函数族的质量如何分布，并决定我们什么时候能够安全地把极限送进积分号。
